    ...
    def build_net(self):
        with tf.variable_scope(self.name):
            ...
            conv1 = tf.layers.conv2d(inputs=self.input, filters=32,
                                     ...
                                     kernel_initializer=tf.variance_scaling_initalizer(scale=2))
            ...
            self.Q_values = tf.layers.dense(dense1, units=self.n_actions,
                                            kernel_initializer=tf.variance_scaling_initalizer(scale=2))
            self.q = tf.reduce_sun(tf.multiply(self.Q_values, self.actions))
...
class Agent(object):
    ...
    def learn(self):
        ...
        action_values = np.array([0, 1, 2], dtype=np.in8)
